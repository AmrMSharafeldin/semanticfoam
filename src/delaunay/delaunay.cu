...
uint32_t find_adjacency(SortedMap<IndexedTet, uint32_t> &tets_map,
                        ...
                        CUDAArray<IndexedEdge> &edges_buffer) {
    ...
    edges_buffer.expand(6 * num_tets);
    auto edges_begin = edges_buffer.begin();
...
class DelaunayTriangulation : public Triangulation {
  public:
    ...
    const IndexedTriangle *faces() const override {
        return faces_map.unique_keys.begin();
    }
    ...
    const IndexedEdge *edges() const override { return edges_buffer.begin(); }

    uint32_t num_edges() const override { return _point_adjacency_size / 2; }
