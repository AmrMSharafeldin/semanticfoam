...
struct IndexedTriangle {
    ...
    /// @brief Get an edge of the triangle
    /// @param i The index of the edge
    RADFOAM_HD IndexedEdge edge(uint32_t i) const {
        switch (i % 3) {
        case 0:
            return IndexedEdge(vertices[0], vertices[1]);
        case 1:
            return IndexedEdge(vertices[0], vertices[2]);
        default:
            return IndexedEdge(vertices[1], vertices[2]);
        }
    }
