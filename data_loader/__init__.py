...
from .segment import SegmentedCOLMAPDataset


dataset_dict = {
    "colmap": COLMAPDataset,
    "segment": SegmentedCOLMAPDataset,
...
class DataHandler:
    ...
    def reload(self, split, downsample=None):
        ...
        if hasattr(split_dataset, "image_paths"):
            # Store full paths and also just the basenames
            self.image_paths = split_dataset.image_paths
            self.image_names = [os.path.basename(p) for p in split_dataset.image_paths]
        elif hasattr(split_dataset, "image_names"):
            self.image_names = split_dataset.image_names
        else:
            # fallback if dataset class doesn't expose names
            self.image_names = [f"frame_{i:05d}.png" for i in range(len(self.rgbs))]
