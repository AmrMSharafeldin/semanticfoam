...
class COLMAPDataset:
    def __init__(self, datadir, split, downsample):
        ...
        images_dir = os.path.join(datadir, f"images_{downsample}" if downsample > 1 else "images")
        masks_dir = os.path.join(datadir, f"masks_{downsample}" if downsample > 1 else "object_mask")
        ...
        self.image_paths = [os.path.join(images_dir, n) for n in names]
        ...
        for image in tqdm(self.images):
            ...
            im = Image.open(os.path.join(images_dir, image.name)).convert("RGB")
            rgbs = torch.tensor(np.array(im), dtype=torch.float32) / 255.0
            im.close()
            
            mask = None
            mask_path = os.path.join(masks_dir, image.name) if os.path.exists(masks_dir) else None
            if mask_path and os.path.exists(mask_path):
                mask = Image.open(mask_path).convert("L")
            else:
                mask = torch.ones((self.img_wh[1], self.img_wh[0]))  # Default mask (all ones)
        ...
        self.all_masks = torch.stack(self.all_masks)
        ...
        self.points3D_color = torch.tensor(np.array(self.points3D_color), dtype=torch.float32) / 255.0