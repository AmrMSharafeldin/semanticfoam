...
py::object trace_forward(Pipeline &self,
    ...
    auto output_id_shape = output_shape;
    output_id_shape.push_back(self.identity_dim()); // TODO change that to self.identity_dim()
    torch::Tensor output_id =
        torch::empty(output_id_shape,
                     torch::dtype(scalar_to_type_meta(self.attribute_type()))
                         .device(rays.device()));
...
py::object trace_backward(Pipeline &self,
    ...
    torch::Tensor ray_grad_in_c = ray_id_grad.contiguous();
    ...
    self.trace_backward(
        ...
        reinterpret_cast<const uint32_t *>(ray_id.data_ptr()),
        reinterpret_cast<const uint32_t *>(ray_id_grad.data_ptr())
...
std::shared_ptr<Pipeline> create_pipeline(int sh_degree,
                                          ...
                                          int identity_dim) {
    return create_pipeline(sh_degree, dtype_to_scalar_type(attr_dtype), identity_dim);
}
...
void init_pipeline_bindings(py::module &module) {
    py::class_<Pipeline, std::shared_ptr<Pipeline>>(module, "Pipeline")
        ...
        .def("trace_backward",
            ...
            py::arg("ray_id") = py::none(),  // ✅ new
            py::arg("ray_id_grad") = py::none(),  // ✅ new
             py::arg("weight_threshold") = py::none(),
             py::arg("max_intersections") = py::none())
        .def("trace_benchmark",
             trace_benchmark,
