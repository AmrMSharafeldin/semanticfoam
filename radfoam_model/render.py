...
class TraceRays(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.id_rgb = results.get("id", None)

        errbox = ErrorBox()
        ctx.errbox = errbox
        #identity_rgb = results["identity_rgb"]  # or however you grab it

        #ctx.mark_non_differentiable(identity_rgb)  # ✅ put this *after* identity_rgb is defined

        ##print(results['id_rgb'].shape)
        
        return (
            results["rgba"],
            results.get("id"),  # identity_rgb
